...
export default function useInsurance() {
  ...
  const submitVehicleDetails = async (data: VehicleFormValues) => {
    try {
      ...
      if (resError.message === "validation error") {
        ...
        toast.error(
          resError.message || "Vehicle details not submitted successfully"
        );
      }
      throw new Error(errorMessage);
    }
  ...
  const submitInsuranceDetails = async (
    /* userData: UserFormValues, */ vehicleData: VehicleFormValues,
    uploadData: DocumentFormValues
  ) => {
    try {
      setLoading(true);
      ...
      const validateFile = (file: File) => {
        if (!validTypes.includes(file.type)) {
          throw new Error(
            "Invalid file type. Only PDF, JPEG, PNG, and JPG are allowed."
          );
        }
      ...
      switch (true) {
        case !!uploadData.utilityBill?.name:
          ...
          document = {
            ...
            type: "utility_bill",
            file: uploadData.utilityBill as File & {
        ...
        case !!uploadData.vehicleLicense?.name:
          ...
          document = {
            ...
            file: uploadData.vehicleLicense as File & {
              type: "application/pdf" | "image/jpeg" | "image/png";
            },
          };
          break;
      ...
      if (document) {
        ...
        return {
          ...
          userId: documentResponse.data.user_id,
        };
      } else {
  ...
  const createBlogSubCategory = async (data: {
    name: string;
  ...
  const updateBlogSubCategory = async (
    id: number,
    data: { name: string; blog_category_id: number }
  ) => {
    try {
      setLoading(true);
  ...
  const createTeam = async (data: BoardData) => {
    try {
      setLoading(true);
      const formData = new FormData();
      Object.keys(data).forEach((key) => {
      ...
      const res = await client.post("/team_members", formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });
      console.log(res);
      router("/admin/dashboard/team");
      return Promise.resolve("Page created successfully");
    } catch (error: any) {
      const resError = error.response?.data;
      const errorMessage = resError?.message || resError?.data;
  ...
  const updateTeam = async (data: BoardData) => {
    try {
      setLoading(true);
      const formData = new FormData();
      Object.keys(data).forEach((key) => {
  ...
  const getTeam = async () => {
    try {
      setLoading(true);
      const res = await client.get(`/team_members`);
      return res?.data?.data;
    } catch (error: any) {
      const resError = error.response?.data;
      const errorMessage = resError?.message || resError?.data;
  ...
  const getTeamById = async (id: number) => {
    try {
      setLoading(true);
      const res = await client.get(`/team_members/${id}`);
      return res?.data?.data;
    } catch (error: any) {
      const resError = error.response?.data;
      const errorMessage = resError?.message || resError?.data;
  ...
  const deleteTeamById = async (id: number) => {
    try {
      setLoading(true);
      const res = await client.delete(`/team_members/${id}`);
      return res?.data?.data;
    } catch (error: any) {
      const resError = error.response?.data;
      const errorMessage = resError?.message || resError?.data;
    ...
    try {
      ...
      Object.keys(data).forEach((key) => {
        ...
        if (key === "image" && data.image instanceof File) {
          ...
          // Skip the image key completely if not a File
          formData.append(key, String(data[key]));
        }
      });
  ...
  const updatePackageSubCategory = async (
    id: number,
    data: PackageSubCategoryValues
  ) => {
    try {
      setLoading(true);
  ...
  const getNewsLettersById = async (id: number) => {
    try {
      ...
      console.log(errorMessage);
      return Promise.reject(
        errorMessage || "Failed to fetch newsletter subscription"
      );
    } finally {
      setLoading(false);
    }
